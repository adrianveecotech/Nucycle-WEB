    ...
    <div class="row ">
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('article.edit_db') }}" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" value={{$id}} name="article_id">
                        <div class="form-group row">
                            <label for="title" class="col-md-4 col-form-label text-md-right">{{ __('Title') }}</label>
                            <div class="col-md-6">
                                <input id="title" type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{ old('title',$article->title)  }}" autofocus>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="start_date" type="datetime-local" value="{{ old('start_date',date('Y-m-d\TH:i',strtotime($article['start_date']))) }}" class="form-control @error('start_date') is-invalid @enderror" name="start_date">
                                @error('start_date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="end_date" type="datetime-local" value="{{ old('end_date',date('Y-m-d\TH:i',strtotime($article['end_date']))) }}" class="form-control @error('end_date') is-invalid @enderror" name="end_date">
                                @error('end_date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6 col-form-label">
                                @if (old('status') == 'Draft' || $article->status == 0)
                                ...
                                @if (old('status') == 'Publish' || $article->status == 1)
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var javaScriptVar = <?php echo str_replace("'","\'",json_encode($article->description)); ?>;
        ...
        function sendFile(file, that) {
            ...
            data.append('locations', 'article_image');
            $.ajaxSetup({
                headers: {
