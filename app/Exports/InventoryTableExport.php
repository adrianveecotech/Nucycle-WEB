...
class InventoryTableExport implements FromCollection,WithHeadings,WithEvents, WithCustomStartCell
{
    private $month = "";
    private $year = "";
    ...
    public function registerEvents(): array {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', "Inventory value - ".$this->month."/".$this->year);
    ...
    public function collection()
    {
        ...
        if (session()->has('monthinnum') && session()->has('year'))
        {
            ...
            foreach($category as $c)
            {
                ...
                $inv->balamt = "RM".number_format($c->balamt, 2, '.', '');
                $inv->purqty = number_format($c->purchaseqty, 2, '.', '');
                $inv->puramt = "RM".number_format($c->purchaseamt, 2, '.', '');
                $inv->avgcost = "RM".number_format($c->avgcost, 2, '.', '');
                $inv->salesqty = number_format($c->salesqty, 2, '.', '');
                $inv->salesamt = "RM".number_format($c->salesamt, 2, '.', '');
                $inv->adjustqty = number_format($c->adjustqty, 2, '.', '');
                $inv->adjustamt = "RM".number_format($c->adjustamt, 2, '.', '');
                $inv->currentqty = number_format($c->currentqty, 2, '.', '');
                $inv->currentamt = "RM".number_format($c->currentamt, 2, '.', '');
        ...
        else
        {
            ...
            foreach($category as $c)
            {
                ...
                $salesqty = round((float)$previous_weight + $current_weight,2);
                $c->salesqty = $salesqty;
                $c->salesamt = round($c->salesqty * $c->avgcost,2);
                $c->adjustqty = ((float)$c->balqty + $c->purchaseqty) - $c->salesqty;
                $c->adjustamt = round($c->adjustqty * $c->avgcost,2);
