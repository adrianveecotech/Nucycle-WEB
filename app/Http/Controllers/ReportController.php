...
class ReportController extends Controller
{
    ...
    public function get_point_hub_weekly_monthly(Request $request)
    {
        ...
        $collectionsMonth = DB::select("SELECT IFNULL(sum(all_point), 0) as total_point,collection_hub.hub_name FROM
                (SELECT * FROM collection WHERE month(created_at) = '$month' and year(created_at) = '$year') AS B
        ...
        foreach ($week as $value) {
            ...
            (SELECT * FROM collection WHERE day(created_at) >= '$value[0]' and day(created_at) <= '$value[1]' and month(created_at) = '$month' and year(created_at) = '$year') AS B
        RIGHT JOIN collection_hub ON B.collection_hub_id = collection_hub.id
        group by collection_hub.id,collection_hub.hub_name");
    ...
    public function get_new_vs_exisiting_recycling_frequency(Request $request)
    {
        ...
        $newUserMonth = DB::select("SELECT count(collection.id) as total from collection left join customer on collection.customer_id = customer.id where year(collection.created_at) = $year and month(collection.created_at) = $month and year(customer.created_at) = $year and month(customer.created_at) = $month");
        $existingUserMonth = DB::select("SELECT count(collection.id) as total from collection left join customer on collection.customer_id = customer.id where year(collection.created_at) = $year and month(collection.created_at) = $month and not (month(customer.created_at) = $month and year(customer.created_at) = $year)");
        ...
        foreach ($week as $value) {
            ...
            $existingUserWeek = DB::select("SELECT count(collection.id) as total from collection left join customer on collection.customer_id = customer.id where month(collection.created_at) = $month and day(collection.created_at) >= '$value[0]' and day(collection.created_at) <= '$value[1]' and not(month(customer.created_at) = $month and day(customer.created_at) >= '$value[0]' and day(customer.created_at) <= '$value[1]')");
    ...
    public function get_collection_collected_transaction_data(Request $request)
    {
        ...
        if ($date_from != '' && $date_to != '') {
            ...
            $stateData = array();
            if ($hub_type == '') {
                $tableData = DB::select("SELECT cus.email, count(cus.email) as total, IF(cus.isIndividual=1, 'B2C', 'B2B') as type from collection c left join customer cus on cus.id = c.customer_id left join collection_hub ch on ch.id = c.collection_hub_id where c.created_at >= '$date_from' and c.created_at <= '$date_to' group by cus.email,cus.isIndividual");
            } else {
                $tableData = DB::select("SELECT cus.email, count(cus.email) as total,IF(cus.isIndividual=1, 'B2C', 'B2B') as type from collection c left join customer cus on cus.id = c.customer_id left join collection_hub ch on ch.id = c.collection_hub_id where ch.type = '$hub_type' and c.created_at >= '$date_from' and c.created_at <= '$date_to' group by cus.email,cus.isIndividual");
            }
        } else {
            if ($hub_type == '') {
                $tableData = DB::select("SELECT cus.email, count(cus.email) as total,IF(cus.isIndividual=1, 'B2C', 'B2B') as type from collection c left join customer cus on cus.id = c.customer_id left join collection_hub ch on ch.id = c.collection_hub_id group by cus.email,cus.isIndividual");
            } else {
                $tableData = DB::select("SELECT cus.email, count(cus.email) as total,IF(cus.isIndividual=1, 'B2C', 'B2B') as type from collection c left join customer cus on cus.id = c.customer_id left join collection_hub ch on ch.id = c.collection_hub_id where ch.type = '$hub_type' group by cus.email,cus.isIndividual");
            }
        }
        if ($format == 'Month') {
            $labels = $months[1];
                ...
                if ($type == 'User') {
                    if ($date_from == '' && $date_to == '') {
                        ...
                        foreach ($months[0] as $value1) {
                            if ($index == 0) {
                                if ($hub_type == '') {
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where  year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");
                                } else {
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'");
                                }
                            ...
                            if ($hub_type == '') {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.state = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));
                            } else {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));
                            }
                        ...
                        foreach ($period as $key => $dt) {
                            ...
                            if ($index == 0) {
                                if ($hub_type == '') {
                                    ...
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where  year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");
                                } else {
                                    array_push($months_name, $dt->format('M y'));
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");
                                }
                            ...
                            $state_id = $value['id'];
                            if ($hub_type == '') {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.state = $state_id and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");
                                $stateData[] = array_column($transactionMonthState, 'total')[0];
                            } else {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = $state_id and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'");
                                $stateData[] = array_column($transactionMonthState, 'total')[0];
                            }
                        }
                        $labels = $months_name;
                    ...
                    if ($date_from == '' && $date_to == '') {
                        ...
                        foreach ($months[0] as $value1) {
                            ...
                            if ($hub_type == '') {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));
                            } else {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$value1'", array($value['id']));
                            }
                        ...
                        foreach ($period as $key => $dt) {
                            ...
                            if ($hub_type == '') {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'", array($value['id']));
                            } else {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$month' and collection.created_at >= '$date_from' and collection.created_at <= '$date_to'", array($value['id']));
                            }
                            $stateCompanyData[] = array_column($companyTransactionMonthState, 'total')[0];
                            $stateIndividualData[] = array_column($individualTransactionMonthState, 'total')[0];
                        }
                ...
                if ($type == 'User') {
                    if ($date_from == '' && $date_to == '') {
                        ...
                        foreach ($weeks as $value1) {
                            if ($index == 0) {
                                if ($hub_type == '') {
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");
                                } else {
                                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 0 and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                    $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 1 and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                    $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");
                                }
                                $companyMonth[] = array_column($companyEachMonth, 'total')[0];
                                $individualMonth[] = array_column($individualEachMonth, 'total')[0];
                                $userMonth[] = array_column($userEachMonth, 'total')[0];
                            ...
                            if ($hub_type == '') {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.state = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                            } else {
                                $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                            }
                            $stateData[] = array_column($transactionMonthState, 'total')[0];
                        }
                        $dataAllStates[] = $stateData;
                        ...
                        foreach ($period as $key => $dt) {
                            ...
                            foreach ($weeks as $key1 => $value1) {
                                ...
                                if ($index == 0) {
                                    ...
                                    if ($hub_type == '') {
                                        $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 0 and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                        $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where customer.isIndividual = 1 and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                        $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");
                                    } else {
                                        $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 0 and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                        $individualEachMonth = DB::select("SELECT count(collection.id)  as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and customer.isIndividual = 1 and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");

                                        $userEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection_hub.type = $hub_type and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'");
                                    }
                                    $companyMonth[] = array_column($companyEachMonth, 'total')[0];
                                    $individualMonth[] = array_column($individualEachMonth, 'total')[0];
                                    $userMonth[] = array_column($userEachMonth, 'total')[0];
                                ...
                                if ($hub_type == '') {
                                    $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.state = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                                } else {
                                    $transactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                                }
                    ...
                    if ($date_from == '' && $date_to == '') {
                        foreach ($weeks as $value1) {
                            ...
                            if ($hub_type == '') {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                            } else {
                                $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                                $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                            }
                            $stateCompanyData[] = array_column($companyTransactionMonthState, 'total')[0];
                            $stateIndividualData[] = array_column($individualTransactionMonthState, 'total')[0];
                        }
                        ...
                        foreach ($period as $key => $dt) {
                            ...
                            foreach ($weeks as $key1 => $value1) {
                                ...
                                if ($hub_type == '') {
                                    $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                                    $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                                } else {
                                    $companyTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 0 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                                    $individualTransactionMonthState = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type  and customer.isIndividual = 1 and collection_hub.hub_state_id = ? and year(collection.created_at) = '$curr_year' and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                                }
        ...
        if ($date_from == '' && $date_to == '') {
            if ($type == "User") {
                foreach ($allCities as $index => $value) {
                    ...
                    if ($format == 'Month') {
                        foreach ($months[0] as $value1) {
                            $transactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.city = ? and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$value1'", array($value['id']));

                            if ($index == 0) {
                                $transactionMonthAllCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$value1'", array($state_id));
                    ...
                    if ($format == 'Week') {
                        foreach ($weeks as $value1) {
                            $transactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.city = ? and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                            if ($index == 0) {
                                $transactionMonthAllCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($state_id));
                                $dataAll[] = array_column($transactionMonthAllCities, 'total')[0];
                            }
                            $dataMonth[] = array_column($transactionMonthCities, 'total')[0];
                        }
            ...
            if ($type == "Hub") {
                ...
                foreach ($hubs as $value) {
                    ...
                    if ($format == "Month") {
                        foreach ($months[0] as $value1) {
                            $companyTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 0 and collection.collection_hub_id = $hub_id and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$value1'", array($value['id']));

                            $individualTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 1 and collection.collection_hub_id = $hub_id and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$value1'", array($value['id']));
                    ...
                    if ($format == "Week") {
                        foreach ($weeks as $value1) {
                            $companyTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 0 and collection.collection_hub_id = $hub_id and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));

                            $individualTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 1 and collection.collection_hub_id = $hub_id and YEAR(collection.created_at) =  YEAR(CURRENT_DATE()) and month(collection.created_at) = '$curr_mon' and day(collection.created_at) >= '$value1[0]' and day(collection.created_at) <= '$value1[1]'", array($value['id']));
                ...
                foreach ($allCities as $index => $value) {
                    ...
                    if ($format == 'Month') {
                        foreach ($period as $key => $dt) {
                            ...
                            $transactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.city = ? and collection.created_at >= '$date_from' and collection.created_at <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year", array($value['id']));

                            if ($index == 0) {
                                array_push($labels, $dt->format('M y'));

                                $transactionMonthAllCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ? and collection.created_at >= '$date_from' and collection.created_at <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year", array($state_id));
                    ...
                    if ($format == 'Week') {
                        foreach ($period as $key => $dt) {
                            ...
                            foreach ($weeks as $key1 => $value1) {
                                ...
                                $transactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.city = ?  and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value1[0] and day(collection.created_at) <= $value1[1]", array($value['id']));

                                if ($index == 0) {
                                    array_push($labels,  $dt->format('Y M ') . $weeksLabel[$key1]);

                                    $transactionMonthAllCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type and customer.state = ?  and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value1[0] and day(collection.created_at) <= $value1[1]", array($state_id));
                                    $dataAll[] = array_column($transactionMonthAllCities, 'total')[0];
                                }
                                $dataMonth[] = array_column($transactionMonthCities, 'total')[0];
                            }
                ...
                foreach ($hubs as $hubIndex => $value) {
                    ...
                    if ($format == "Month") {
                        ...
                        foreach ($period as $key => $dt) {
                            ...
                            $companyTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 0 and collection.collection_hub_id = $hub_id and collection.created_at >= '$date_from' and collection.created_at <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year", array($value['id']));

                            $individualTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 1 and collection.collection_hub_id = $hub_id and collection.created_at >= '$date_from' and collection.created_at <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year", array($value['id']));
                    ...
                    if ($format == "Week") {
                        foreach ($period as $key => $dt) {
                            ...
                            foreach ($weeks as $key1 => $value1) {
                                ...
                                if ($hubIndex == 0)
                                    array_push($labels, $dt->format('Y M ') . $weeksLabel[$key1]);
                                $companyTransactionMonthCities = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id left join state on customer.state = state.id where collection_hub.type = $hub_type   and customer.isIndividual = 0 and collection.collection_hub_id = $hub_id and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value1[0] and day(collection.created_at) <= $value1[1]", array($value['id']));
    ...
    public function get_collection_waste_selling_data(Request $request)
    {
        ...
        if ($date_from == '' && $date_to == '') {
            if ($format == 'Month') {
                foreach ($months[0] as  $index => $value) {
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $value and year(c.created_at) = $curr_year  and status = 1");
                        ...
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $value and year(c.created_at) = $curr_year  and chb.type = $hub_type and status = 1");
            ...
            if ($format == 'Week') {
                foreach ($weeks as $value) {
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value[0] and day(c.created_at) <= $value[1] and status = 1");
                        ...
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value[0] and day(c.created_at) <= $value[1] and chb.type = $hub_type and status = 1");
            ...
            if ($format == 'Month') {
                foreach ($period as $key => $dt) {
                    ...
                    array_push($labels, $dt->format('M Y'));
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year and status = 1");
                        ...
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year  and chb.type = $hub_type and status = 1");
            ...
            if ($format == 'Week') {
                foreach ($period as $key => $dt) {
                    ...
                    foreach ($weeks as $key1 => $value1) {
                        ...
                        array_push($labels, $dt->format('Y M ') . $weeksLabel[$key1]);
                        if ($hub_type == '') {
                            $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] and status = 1");

                            $soldWaste = DB::select("SELECT IFNULL(round(sum(wci.weight),2),0) as total from waste_clearance_schedule wcs left join waste_clearance_item wci on wcs.id = wci.waste_clearance_schedule_id left join collection_hub chb on chb.id = wcs.collection_hub_id where month(wcs.completed_at) = $curr_mon and year(wcs.completed_at) = $curr_year and day(wcs.completed_at) >= $value1[0] and day(wcs.completed_at) <= $value1[1] and wcs.status = 2");
                        } else {
                            $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] and chb.type = $hub_type and status = 1");
            ...
            if ($format == 'Month') {
                foreach ($category as $value) {
                    ...
                    foreach ($months[0] as $value1) {
                        ...
                        $collectedWasteEvery = DB::select("SELECT ifnull(round(sum(B.weight),2),0) as total, ch.hub_name as name FROM (SELECT c.id,cd.weight,c.collection_hub_id from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id left join recycle_type rt on cd.recycling_type_id = rt.id where month(c.created_at) = $value1 and year(c.created_at) = $curr_year and rt.recycle_category_id = $value->id and chb.hub_state_id = $state_id and chb.type = $hub_type and c.status = 1) AS B RIGHT JOIN collection_hub ch on ch.id = B.collection_hub_id where ch.hub_state_id = $state_id and ch.type = $hub_type group by ch.id,ch.hub_name order by ch.id");
            ...
            if ($format == 'Week') {
                foreach ($category as $value) {
                    ...
                    foreach ($weeks as $value1) {
                        ...
                        $collectedWasteEvery = DB::select("SELECT ifnull(round(sum(B.weight),2),0) as total, ch.hub_name as name FROM (SELECT c.id,cd.weight,c.collection_hub_id from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id left join recycle_type rt on cd.recycling_type_id = rt.id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] and rt.recycle_category_id = $value->id and chb.hub_state_id = $state_id and chb.type = $hub_type and c.status = 1) AS B RIGHT JOIN collection_hub ch on ch.id = B.collection_hub_id where ch.hub_state_id = $state_id and ch.type = $hub_type group by ch.id,ch.hub_name order by ch.id");
            ...
            if ($format == 'Month') {
                foreach ($category as $index => $value) {
                    ...
                    foreach ($period as $key => $dt) {
                        ...
                        $collectedWasteEvery = DB::select("SELECT ifnull(round(sum(B.weight),2),0) as total, ch.hub_name as name FROM (SELECT c.id,cd.weight,c.collection_hub_id from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id left join recycle_type rt on cd.recycling_type_id = rt.id where c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year and rt.recycle_category_id = $value->id and chb.hub_state_id = $state_id and chb.type = $hub_type and c.status = 1) AS B RIGHT JOIN collection_hub ch on ch.id = B.collection_hub_id where ch.hub_state_id = $state_id and ch.type = $hub_type group by ch.id,ch.hub_name order by ch.id");
    ...
    public function get_active_transaction(Request $request)
    {
        ...
        if ($date_from == '' && $date_to == '') {
            $tableData = DB::select("SELECT DISTINCT(c.email),IF(c.isIndividual=1, 'B2C', 'B2B') as type, concat(year(co.created_at),'-',month(co.created_at)) as date from collection co left join customer c on c.id = co.customer_id");
            if ($format == 'Month') {
                $labels = $months[1];
                foreach ($allStates as $index1 => $value1) {
                    $state_id = $value1['id'];
                    ...
                    foreach ($months[0] as  $index => $value) {
                        if ($index1 == 0) {
                            $individualMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 1 and month(co.created_at) = $value and year(co.created_at) = $curr_year");

                            $companyMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 0 and month(co.created_at) = $value and year(co.created_at) = $curr_year");

                            $totalMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where month(co.created_at) = $value and year(co.created_at) = $curr_year");
                        ...
                        $totalMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.state = $state_id and month(co.created_at) = $value and year(co.created_at) = $curr_year");
                        $stateData[] = array_column($totalMonth, 'total')[0];
                    }
                    $dataAllStates[] = $stateData;
                    $allStatesName[] = $value1['name'];
            ...
            if ($format == 'Week') {
                ...
                foreach ($allStates as $index1 => $value1) {
                    ...
                    foreach ($weeks as $value) {
                        if ($index1 == 0) {
                            $individualWeek = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 1 and month(co.created_at) = $curr_mon and year(co.created_at) = $curr_year and day(co.created_at) >= $value[0] and day(co.created_at) <= $value[1]");

                            $companyWeek = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 0 and month(co.created_at) = $curr_mon and year(co.created_at) = $curr_year and day(co.created_at) >= $value[0] and day(co.created_at) <= $value[1]");

                            $totalWeek = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where month(co.created_at) = $curr_mon and year(co.created_at) = $curr_year and day(co.created_at) >= $value[0] and day(co.created_at) <= $value[1]");

                            array_push($individual, array_column($individualWeek, 'total')[0]);
                            array_push($company, array_column($companyWeek, 'total')[0]);
                            array_push($total, array_column($totalWeek, 'total')[0]);
                        }
                        $totalWeek = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where  c.state = $state_id and c.isIndividual = 1 and month(co.created_at) = $curr_mon and year(co.created_at) = $curr_year and day(co.created_at) >= $value[0] and day(co.created_at) <= $value[1]");
                        $stateData[] = array_column($totalWeek, 'total')[0];
                    }
                    $dataAllStates[] = $stateData;
                    $allStatesName[] = $value1['name'];
            ...
            $monthOfEndDate = date('m', strtotime($date_to));
            $labels = array();
            $tableData = DB::select("SELECT DISTINCT(c.email),IF(c.isIndividual=1, 'B2C', 'B2B') as type, concat(year(co.created_at),'-',month(co.created_at)) as date from collection co left join customer c on c.id = co.customer_id where co.created_at >= '$date_from' and co.created_at <= '$date_to'");
            if ($format == 'Month') {
                foreach ($allStates as $index1 => $value1) {
                    $state_id = $value1['id'];
                    $stateData = array();
                    foreach ($period as $key => $dt) {
                        ...
                        if ($index1 == 0) {
                            ...
                            $individualMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 1 and month(co.created_at) = $month and year(co.created_at) = $year and co.created_at >= '$date_from' and co.created_at <= '$date_to'");

                            $companyMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.isIndividual = 0 and month(co.created_at) = $month and year(co.created_at) = $year and co.created_at >= '$date_from' and co.created_at <= '$date_to'");

                            $totalMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where month(co.created_at) = $month and year(co.created_at) = $year and co.created_at >= '$date_from' and co.created_at <= '$date_to'");
                        ...
                        $totalMonth = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.state = $state_id and month(co.created_at) = $month and year(co.created_at) = $year and co.created_at >= '$date_from' and co.created_at <= '$date_to'");
                        $stateData[] = array_column($totalMonth, 'total')[0];
                    }
                    $dataAllStates[] = $stateData;
                    $allStatesName[] = $value1['name'];
        ...
        if ($date_from == '' && $date_to == '') {
            foreach ($allCities as $index => $value) {
                ...
                if ($format == 'Month') {
                    ...
                    foreach ($months[0] as $value1) {
                        $city_id = $value['id'];
                        $dataCities = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.city = $city_id and month(co.created_at) = $value1 and year(co.created_at) = $curr_year  ");

                        if ($index == 0) {
                            $dataStates = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.state = $state_id and month(co.created_at) = $value1 and year(co.created_at) = $curr_year ");
                            $dataAll[] = array_column($dataStates, 'total')[0];
                        }
                        $dataMonth[] = array_column($dataCities, 'total')[0];
                    }
                ...
                if ($format == 'Week') {
                    ...
                    foreach ($weeks as $value1) {
                        $city_id = $value['id'];
                        $dataCities = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.city = $city_id and year(co.created_at) = $curr_year and month(co.created_at) = $curr_mon and day(co.created_at) >= $value1[0] and day(co.created_at) <= $value1[1] ");

                        if ($index == 0) {
                            $dataStates = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.state = $state_id and year(co.created_at) = $curr_year and month(co.created_at) = $curr_mon and day(co.created_at) >= $value1[0] and day(co.created_at) <= $value1[1]");
                            $dataAll[] = array_column($dataStates, 'total')[0];
                        }
                        $dataMonth[] = array_column($dataCities, 'total')[0];
                    }
            ...
            foreach ($allCities as $index => $value) {
                ...
                if ($format == 'Month') {
                    foreach ($period as $key => $dt) {
                        ...
                        $city_id = $value['id'];
                        $dataCities = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.city = $city_id and co.created_at >= '$date_from' and co.created_at <= '$date_to' and month(co.created_at) = $month and year(co.created_at) = $year  ");

                        if ($index == 0) {
                            array_push($labels, $dt->format('M Y'));
                            $dataStates = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.state = $state_id and month(co.created_at) = $month and year(co.created_at) = $year and co.created_at >= '$date_from' and co.created_at <= '$date_to'");
                            $dataAll[] = array_column($dataStates, 'total')[0];
                        }
                        $dataMonth[] = array_column($dataCities, 'total')[0];
                    }
                ...
                if ($format == 'Week') {
                    foreach ($period as $key => $dt) {
                        ...
                        foreach ($weeks as $key1 => $value1) {
                            ...
                            $city_id = $value['id'];
                            $dataCities = DB::select("SELECT count(DISTINCT(co.customer_id)) as total from collection co left join customer c on co.customer_id = c.id where c.city = $city_id and year(co.created_at) = $curr_year and month(co.created_at) = $curr_mon and day(co.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] ");
        ...
        if ($format == 'Month') {
            if ($date_from == '' && $date_to == '') {
                foreach ($months as $value) {
                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 0 and month(collection.created_at) = $value and year(collection.created_at) = $curr_year");

                    $individualEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 1 and month(collection.created_at) = $value and year(collection.created_at) = $curr_year");

                    $totalEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and month(collection.created_at) = $value and year(collection.created_at) = $curr_year");
                ...
                foreach ($period as $key => $dt) {
                    ...
                    $companyEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 0 and (collection.created_at) >= '$date_from' and (collection.created_at) <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year ");

                    $individualEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 1 and (collection.created_at) >= '$date_from' and (collection.created_at) <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year ");

                    $totalEachMonth = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and (collection.created_at) >= '$date_from' and (collection.created_at) <= '$date_to' and month(collection.created_at) = $month and year(collection.created_at) = $year ");
            ...
            if ($date_from == '' && $date_to == '') {
                foreach ($weeks as $value) {
                    $companyEachWeek = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 0 and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value[0] and day(collection.created_at) <= $value[1]");

                    $individualEachWeek = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and customer.isIndividual = 1 and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value[0] and day(collection.created_at) <= $value[1]");

                    $totalEachWeek = DB::select("SELECT count(collection.id) as total FROM collection left join collection_hub on collection.collection_hub_id = collection_hub.id left join customer on collection.customer_id = customer.id where collection.collection_hub_id = $collection_hub_id and month(collection.created_at) = $curr_mon and year(collection.created_at) = $curr_year and day(collection.created_at) >= $value[0] and day(collection.created_at) <= $value[1] ");
    ...
    public function collectionhub_collected_waste_data(Request $request)
    {
        ...
        if ($format == 'Month') {
            if ($date_from == '' && $date_to == '') {
                foreach ($months as $value) {
                    $categoryEachMonth = DB::select("SELECT round(ifnull(sum(B.weight),0),2) as total FROM (SELECT cd.weight,rt.recycle_category_id from collection c left join collection_detail cd on c.id = cd.collection_id left join recycle_type rt on cd.recycling_type_id = rt.id where month(c.created_at) = $value and year(c.created_at) = $curr_year and c.collection_hub_id = $collection_hub_id) AS B RIGHT JOIN recycle_category rc on rc.id = B.recycle_category_id group by rc.id");
                ...
                foreach ($period as $key => $dt) {
                    ...
                    $categoryEachMonth = DB::select("SELECT round(ifnull(sum(B.weight),0),2) as total FROM (SELECT cd.weight,rt.recycle_category_id from collection c left join collection_detail cd on c.id = cd.collection_id left join recycle_type rt on cd.recycling_type_id = rt.id where c.collection_hub_id = $collection_hub_id and c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year) AS B RIGHT JOIN recycle_category rc on rc.id = B.recycle_category_id group by rc.id");
            ...
            if ($date_from == '' && $date_to == '') {
                foreach ($weeks as $value) {
                    $categoryEachWeek = DB::select("SELECT round(ifnull(sum(B.weight),0),2) as total FROM (SELECT cd.weight,rt.recycle_category_id from collection c left join collection_detail cd on c.id = cd.collection_id left join recycle_type rt on cd.recycling_type_id = rt.id where c.collection_hub_id = $collection_hub_id and month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value[0] and day(c.created_at) <= $value[1]) AS B RIGHT JOIN recycle_category rc on rc.id = B.recycle_category_id group by rc.id");
    ...
    public function collectionhub_waste_selling_data(Request $request)
    {
        ...
        if ($date_from == '' && $date_to == '') {
            if ($format == 'Month') {
                foreach ($months[0] as  $index => $value) {
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $value and year(c.created_at) = $curr_year and status = 1 and c.collection_hub_id = $collection_hub_id");

                        $soldWaste = DB::select("SELECT IFNULL(round(sum(wci.weight),2),0) as total from waste_clearance_schedule wcs left join waste_clearance_item wci on wcs.id = wci.waste_clearance_schedule_id left join collection_hub chb on chb.id = wcs.collection_hub_id where month(wcs.completed_at) = $value and year(wcs.completed_at) = $curr_year and wcs.status = 2 and wcs.collection_hub_id = $collection_hub_id");
                    } else {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $value and year(c.created_at) = $curr_year  and chb.type = $hub_type and status = 1 and c.collection_hub_id = $collection_hub_id");
            ...
            if ($format == 'Week') {
                foreach ($weeks as $value) {
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value[0] and day(c.created_at) <= $value[1] and status = 1 and c.collection_hub_id = $collection_hub_id");

                        $soldWaste = DB::select("SELECT IFNULL(round(sum(wci.weight),2),0) as total from waste_clearance_schedule wcs left join waste_clearance_item wci on wcs.id = wci.waste_clearance_schedule_id left join collection_hub chb on chb.id = wcs.collection_hub_id where month(wcs.completed_at) = $curr_mon and year(wcs.completed_at) = $curr_year and day(wcs.completed_at) >= $value[0] and day(wcs.completed_at) <= $value[1] and wcs.status = 2 and wcs.collection_hub_id = $collection_hub_id");
                    } else {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value[0] and day(c.created_at) <= $value[1] and chb.type = $hub_type and status = 1 and c.collection_hub_id = $collection_hub_id");
            ...
            if ($format == 'Month') {
                foreach ($period as $key => $dt) {
                    ...
                    array_push($labels, $dt->format('M Y'));
                    if ($hub_type == '') {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year and status = 1 and c.collection_hub_id = $collection_hub_id");

                        $soldWaste = DB::select("SELECT IFNULL(round(sum(wci.weight),2),0) as total from waste_clearance_schedule wcs left join waste_clearance_item wci on wcs.id = wci.waste_clearance_schedule_id left join collection_hub chb on chb.id = wcs.collection_hub_id where wcs.completed_at >= '$date_from' and wcs.completed_at <= '$date_to' and month(wcs.completed_at) = $month and year(wcs.completed_at) = $year and wcs.status = 2 and wcs.collection_hub_id = $collection_hub_id");
                    } else {
                        $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where c.created_at >= '$date_from' and c.created_at <= '$date_to' and month(c.created_at) = $month and year(c.created_at) = $year  and chb.type = $hub_type and status = 1 and c.collection_hub_id = $collection_hub_id");
            ...
            if ($format == 'Week') {
                foreach ($period as $key => $dt) {
                    ...
                    foreach ($weeks as $key1 => $value1) {
                        ...
                        array_push($labels, $dt->format('Y M ') . $weeksLabel[$key1]);
                        if ($hub_type == '') {
                            $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] and status = 1 and c.collection_hub_id = $collection_hub_id");

                            $soldWaste = DB::select("SELECT IFNULL(round(sum(wci.weight),2),0) as total from waste_clearance_schedule wcs left join waste_clearance_item wci on wcs.id = wci.waste_clearance_schedule_id left join collection_hub chb on chb.id = wcs.collection_hub_id where month(wcs.completed_at) = $curr_mon and year(wcs.completed_at) = $curr_year and day(wcs.completed_at) >= $value1[0] and day(wcs.completed_at) <= $value1[1] and wcs.status = 2 and wcs.collection_hub_id = $collection_hub_id");
                        } else {
                            $collectedWaste = DB::select("SELECT IFNULL(round(sum(cd.weight),2),0) as total from collection c left join collection_detail cd on c.id = cd.collection_id left join collection_hub chb on chb.id = c.collection_hub_id where month(c.created_at) = $curr_mon and year(c.created_at) = $curr_year and day(c.created_at) >= $value1[0] and day(c.created_at) <= $value1[1] and chb.type = $hub_type and status = 1 and c.collection_hub_id = $collection_hub_id");
